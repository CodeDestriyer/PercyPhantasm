    ...
    <section className="max-w-6xl mx-auto px-6 py-14 md:py-20">
      <div className="grid md:grid-cols-12 gap-10 items-center">
        <div className="md:col-span-7">
          <p className="smallcaps text-xs mb-4 ornament inline-block">
            Sensational Apparatus
          </p>
          <h2 className="font-display font-black text-5xl md:text-6xl leading-[1.05]">
            Кинематографъ-<span className="italic">Рѣзакъ</span>
          </h2>
          <p className="mt-5 text-lg md:text-xl text-ink/80 max-w-xl">
            Механическій ассистентъ, обращающій длинныя фильмы въ карманныя
            миніатюры для всеобщаго увеселенія публики въ электрическихъ сѣтяхъ.
          </p>
          <div className="mt-8 flex flex-wrap gap-4">
            <button onClick={onLaunch} className="btn-engraved">
              <span>Запустить аппаратъ</span>
              <span aria-hidden>→</span>
            </button>
            <a href="#principle" className="btn-engraved !bg-ink !text-paper">
              Принципъ дѣйствія
            </a>
          </div>
          <dl className="mt-10 grid grid-cols-3 gap-6 max-w-md">
            {[
              ["IX:XVI", "Формать вертикалій"],
              ["∞", "Длительность ленты"],
              ["I мин.", "Среднее время обработки"],
            ].map(([n, l]) => (
              <div key={l} className="border-t-2 border-ink pt-2">
                <dt className="font-display text-2xl">{n}</dt>
                <dd className="text-xs smallcaps text-ink/70 mt-1">{l}</dd>
              </div>
            ))}
          </dl>
        </div>
...
function PatentDrawing() {
  return (
    <figure className="border-2 border-ink p-4 bg-cream/40">
      ...
      <figcaption className="mt-3 text-center text-xs smallcaps">
        Чертёжъ № 1 — Общій видъ
      </figcaption>
    </figure>
  );
}
